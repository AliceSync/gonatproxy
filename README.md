...
- 默认监听 **`0.0.0.0:8443`**（未配置 `listen` 时），中继与控制台**同端口共享**：
  服务器按连接区分 HTTP（控制台）与隧道（中继），浏览器和 AI 客户端共用同一 8443/TLS。
- 也可通过 `admin.listen` 给控制台独立端口（如 `":8445"`）；留空则共享中继端口。
- 所有模式都会**显示正确的监听地址**（`start` 后台启动会在切入后台前打印 relay/console
  地址；前台启动打印实际配置值；不会把 `0.0.0.0` 错显示成 `127.0.0.1`）。若端口被占用，
  前台/`start` 会**直接打印错误并以非零退出**，而非静默后台挂起。
- **不使用随机/临时端口回退**：端口被占用就是硬错误。`start` 会在**切入后台前在前台
  预检**要绑定的端口是否可用，不可用立即报错退出，避免"看起来起来了其实没起来"。
...
- **单页应用（SPA）导航**：各页面共用同一个外壳，点击顶部导航（以及页面内指向
  `dashboard/config/files/deploy/services/system` 的链接）时**只替换页面主体，不再整页刷新**；
  地址栏用 `history.pushState` 更新，浏览器的前进/后退可用，会话过期会自动回到登录页。
  顶部有细加载条。切换页面时若上一页有活动连接会先清理（如概览页的实时指标 SSE）。
- **概览**：运行模式（前台 / start 守护 / systemd 管理）、监听、路由、NAT 客户端汇总。
  **实时指标**通过 **SSE（Server-Sent Events，`/api/metrics/stream`）** 实时推送——服务器每
  秒推送一次连接数（入口/NAT/总）与上/下行带宽快照，浏览器**不再每秒发请求轮询**
  （`/api/metrics` 仍保留用于一次性拉取）。连接数来自实时连接计数，带宽由中继字节计数按秒
  求导得出，图形为最近 60 秒总吞吐。
...
- **系统**：更新程序 + 重启。上传新的 `deepseek-server`，选择是否上传后立即替换重启
  ...
  - **重启始终采用 fork**（不再检查/委托 systemctl）：即使你运行在某个
    systemctl 服务派生出的 shell 里再用 `start` 启动，也能正常重启，不会被
    `systemctl restart` 误判到不属于你的单位而失败。systemd 的生命周期控制只在
    **服务**页的启用/启动/停止/重启等按钮里显式使用。
  - **配置保存即生效**：改 `cert_file`/`key_file` 后保存，证书热切换到新位置
    （无需重启进程）；路由/secret 等也沿用热重载。
  - **操作始终有反馈**：重启会对请求**先返回** `restarting` 提示，再异步完成
    停止旧进程并 fork 新进程（从而你能看到"正在重启…"，而不是点了没反应）。
...
- **client / natclient 重连采用指数退避 + ±20% 抖动**：从配置的基准延迟（默认 5s）
  在快速重连时逐次翻倍至上限 60s，连接稳定 30s 后重置。避免抖动期间反复建连，
  也避免大量端点同时恢复造成惊群（thundering herd）。
- **server 对空闲连接设置注册超时**：TLS 握手/等待 REGISTER 的内联连接会在有限时间内
  被丢弃（共享端口另有 10s 握手超时），不会无限占用 goroutine 与文件描述符。
- **server 共享端口限制并发 TLS 握手数**（默认 256）：面对半开连接洪泛时，超出即丢弃，
  不会耗尽 cgroup/fd/goroutine。
- **部署助手生成的安装命令带网络韧性**：`curl` 增加 `--retry 5 --retry-delay 4
  --retry-all-errors --connect-timeout 10 --max-time`，断网自动重试且不陷入无限重试/卡死。
