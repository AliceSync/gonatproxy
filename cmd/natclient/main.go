...
func main() {
	...
	for {
		...
		// outage the retry interval grows from the base up to a cap, and jitter
		// plus the cap prevents a fleet of NAT hosts from thundering-herding the
		// server with a burst of connections when the network recovers.
		base := reconnect
		delay := base
